"""
Este módulo contiene la clase GeoUtils, que proporciona diversas funciones 
para trabajar con datos geográficos. La clase cuenta con una clase que 
contiene métodos para geocodificar una dirección, crear un mapa de calor y
crear un mapa con centroides de agrupación.

Clase:
    GeoUtils
    
Métodos:
    geocode_address(address): Geocodifica una dirección a partir de un string.
    
    heatmap(user_routes): Crea un mapa de calor a partir de las rutas de un 
    usuario.
    
    clustermap(user_routes, num_clusters=1): Crea un mapa con centroides de 
    agrupación de las rutas de un usuario.
...
class GeoUtils:
    ...
    Métodos:
        geocode_address(address): Geocodifica una dirección a partir de un
        string.
        ...
        clustermap(user_routes, num_clusters=1): Crea un mapa con centroides de
        agruación de las rutas de un usuario.
    """
    ...
    def heatmap(self, user_routes):
        ...
        # Iterar sobre las rutas del usuario y extraer las coordenadas
        ...
        gdf = gpd.GeoDataFrame(
            geometry=gpd.points_from_xy(
                [coord[1] for coord in coords], [coord[0] for coord in coords]
            )
        )
        ...
        HeatMap(data=[[point.y, point.x] for point in gdf.geometry], radius=15).add_to(
            m
        )
    ...
    def clustermap(self, user_routes, num_clusters=1):
        ...
        gdf = gpd.GeoDataFrame(
            geometry=gpd.points_from_xy(
                [coord[1] for coord in coordinates], [coord[0] for coord in coordinates]
            )
        )
