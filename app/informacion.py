...
def vehiculos():
    ...
    return pd.read_csv(ruta_archivo, names=['id', 'placa', 'tipo', 'modelo', 'año', 'capacidad', 'kilometraje', 'combustible', 'rendimiento'])
...
def pedidos():
    ...
    return pd.read_csv(ruta_archivo, names=['direccion', 'fecha', 'cliente', 'estado'])

def acciones_conductor_combustible():
    ruta_archivo = os.path.join(direccion_actual, 'bdd', 'acciones_conductor_combustible.csv')
    return pd.read_csv(ruta_archivo, names=['conductor', 'vehiculo', 'galones_gasolina', 'kilometraje', 'fecha', 'hora'])

def geocode_pedidos():
    return pd.read_csv('bdd/geocode_pedidos.csv', names=['direccion', 'latitude', 'longitude'])
