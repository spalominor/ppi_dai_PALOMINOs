...
from utils import obtener_graficos
...
def main():
    st.title("Gráficos")
    st.write("Selecciona el tema y tipo de gráfico que deseas visualizar")
...
def vista(tipo_grafico):
    if tipo_grafico == "Pedidos según el día y la hora":
        graficar_pedidos()
    elif tipo_grafico == "Rendimiento de combustible de los vehículos":
...
def graficar_pedidos():
    if st.checkbox("Gráfico de pastel: Pedidos por día"):
        st.write("Cantidad de pedidos realizados según el día de la semana")
        fig = obtener_graficos.graficar_pastel_pedidos_dia()
        st.pyplot(fig)
    if st.checkbox("Histograma: Pedidos por hora del día"):
        st.write("Cantidad de pedidos realizados cada dos horas")
        fig = obtener_graficos.graficar_histograma_pedidos_por_hora()
        st.pyplot(fig)
        
        
def graficar_kilometros_ano():
    if st.checkbox("Grafico de barras: Kilómetros recorridos por año"):
        st.write("Cantidad de kilómetros en promedio recorridos por año por cada vehículo según la antigüedad")
        fig = obtener_graficos.graficar_barras_kilometros_ano()
        st.pyplot(fig)
    elif st.checkbox("Grafico boxplot: Kilómetros recorridos por año"):
        st.write("Distribución de los kilómetros recorridos por año")
        fig = obtener_graficos.graficar_boxplot_kilometros_ano()
        st.pyplot(fig)
...
def graficar_rendimiento_combustible():
    if st.checkbox("Gráfico de barras: Rendimiento de los vehículos"):
        ...
        fig = obtener_graficos.graficar_barras_rendimiento_combustible()
        st.pyplot(fig)
    elif st.checkbox("Grafico boxplot: Rendimiento de los vehículos"):
        st.write("Distribución del rendimiento de los vehículos")
        fig = obtener_graficos.graficar_boxplot_rendimiento_combustible()
        st.pyplot(fig)
    elif st.checkbox("Grafico de dispersión: Rendimiento de los vehículos"):
        st.write("Relación entre la antigüedad y el rendimiento de los vehículos")
        fig = obtener_graficos.graficar_scatter_rendimiento_combustible()
        st.pyplot(fig)
...
def graficar_kilometraje_vehiculos():
    if st.checkbox("Gráfico de histograma: Kilometraje de los vehículos"):
        ...
        cortes = int(st.text_input("Ingrese el número de cortes", 12))
        fig = obtener_graficos.graficar_histograma_kilometraje_vehiculos(cortes)
        st.pyplot(fig)
    elif st.checkbox("Grafico boxplot: Kilometraje de los vehículos"):
        st.write("Distribución del kilometraje de los vehículos")
        fig = obtener_graficos.graficar_boxplot_kilometraje_vehiculos()
        st.pyplot(fig)
...
def graficar_autonomia_vehiculos():
    if st.checkbox("Gráfico de barras: Autonomía de los vehículos"):
        ...
        fig = obtener_graficos.graficar_barras_autonomia_vehiculos()
        st.pyplot(fig)
    elif st.checkbox("Grafico boxplot: Autonomía de los vehículos"):
        st.write("Distribución de la autonomía de los vehículos")
        fig = obtener_graficos.graficar_boxplot_autonomia_vehiculos()
        st.pyplot(fig)
    elif st.checkbox("Grafico de dispersión: Autonomía de los vehículos"):
        st.write("Relación entre la autonomía y la capacidad de combustible de los vehículos")
        fig = obtener_graficos.graficar_scatter_autonomia_vehiculos()
        st.pyplot(fig)
